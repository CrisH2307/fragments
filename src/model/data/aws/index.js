...
async function deleteFragment(ownerId, id) {
  ...
  logger.debug({ s3params }, 'S3 delete parameters');
  logger.debug({ ddbparams }, 'DynamoDB delete parameters');

  try {
    return await Promise.all([ddbDocClient.send(ddbcommand), s3Client.send(s3command)]);
  } catch (error) {
    logger.error(
      { error, s3params, ddbparams },
      'Error occurred during fragment deletion from S3 or DynamoDB'
    );
